...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'INSERT INTO post (title,location,category, body, author_id)'
                ' VALUES (?,?,? ?, ?)',
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'UPDATE post SET title = ?, location = ?, category = ?, body = ?'
                ' WHERE id = ?',
                (title, location, category, body, id)
            )
            db.commit()
            return redirect(url_for('blog.index'))
